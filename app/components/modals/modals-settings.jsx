...
// Top-level (not nested inside SettingsModal) so React keeps the same component identity across
// re-renders — a component declared inside another component's render body gets a fresh function
// reference every render, which React treats as an entirely different type and remounts from
// scratch on every parent update. That was collapsing TypeEditor's `unlocked` state (and any
// unblurred input) back to locked every time a single field was committed anywhere in the modal —
// the "can't edit several fields", "rollback" bug.
const settingsInputStyle = { padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 4, fontSize: 13.5, fontFamily: 'inherit', background: 'var(--surface)', color: 'var(--ink)', boxSizing: 'border-box' };

function SettingsToggle({ label, on, onClick }) {
  return (
...
function SettingsNumField({ label, value, onCommit, step, width }) {
  return (
    <label style={{ display: 'block' }}>
      ...
      <input type="number" step={step || 1} defaultValue={value} onBlur={e => onCommit(e.target.value === '' ? '' : Number(e.target.value))}
        style={{ ...settingsInputStyle, width: width || '100%' }} />
    </label>
  );
}

function TypeEditor({ firm, t, onUpdate, onRemove }) {
  const [unlocked, setUnlocked] = useStateSet(!t.label);
  const patch = (p) => onUpdate(firm, t.id, p);
      ...
      <div style={{ padding: 14, borderRadius: 4, background: 'var(--surface)', border: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button className="tj-iconbtn tj-del" style={{ width: 34, height: 34 }} title="Supprimer ce type" onClick={() => onRemove(firm, t.id)}><window.Icon name="trash" size={15} /></button>
          </div>
        </div>
    ...
    <div style={{ padding: 14, borderRadius: 4, background: 'var(--surface)', border: '1px solid var(--ink)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, alignItems: 'end' }}>
        <label style={{ display: 'block' }}>
          ...
          <input defaultValue={t.label} placeholder="ex. 50K EOD" onBlur={e => patch({ label: e.target.value })} style={{ ...settingsInputStyle, width: '100%' }} />
        </label>
        <SettingsNumField label="Taille ($)" value={t.size} step={5000} onCommit={v => patch({ size: v || 0 })} />
        <button className="tj-iconbtn tj-del" style={{ width: 34, height: 34 }} title="Supprimer ce type" onClick={() => onRemove(firm, t.id)}><window.Icon name="trash" size={15} /></button>
      </div>
      ...
      <div style={{ marginTop: 12, padding: 12, borderRadius: 4, background: 'var(--surface-2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <label style={{ display: 'block' }}>
            ...
            <select defaultValue={t.ddType || 'none'} onChange={e => patch({ ddType: e.target.value })} style={{ ...settingsInputStyle, width: '100%' }}>
              <option value="none">Aucun</option>
              <option value="trailing">Suiveur</option>
              <option value="static">Statique</option>
            </select>
          </label>
          <SettingsNumField label="Montant ($)" value={t.ddAmount} step={100} onCommit={v => patch({ ddAmount: v || 0 })} />
          <SettingsNumField label="Se fige à ($, vide = illimité)" value={t.ddStop} step={100} onCommit={v => patch({ ddStop: v })} />
        </div>
      </div>

      {/* DLL */}
      <div style={{ marginTop: 10, padding: 12, borderRadius: 4, background: 'var(--surface-2)' }}>
        <SettingsToggle label="Limite de perte journalière (DLL)" on={!!t.hasDll} onClick={() => patch({ hasDll: !t.hasDll })} />
        {t.hasDll && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--ink-2)', marginTop: 10 }}>
            <span>Max</span>
            <input type="number" step="50" defaultValue={t.dllAmount} onBlur={e => patch({ dllAmount: Number(e.target.value) || 0 })} style={{ ...settingsInputStyle, width: 80, textAlign: 'center' }} />
      ...
      {/* inactivity rule */}
      <div style={{ marginTop: 10, padding: 12, borderRadius: 4, background: 'var(--surface-2)' }}>
        <SettingsToggle label="Règle d'inactivité" on={!!t.hasInactivity} onClick={() => patch({ hasInactivity: !t.hasInactivity })} />
        {t.hasInactivity && (
          <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 13, color: 'var(--ink-2)' }}>
              <span>Min.</span>
              <input type="number" min="1" defaultValue={t.inactMinQualDays} onBlur={e => patch({ inactMinQualDays: Number(e.target.value) || 1 })} style={{ ...settingsInputStyle, width: 56, textAlign: 'center' }} />
              <span>j à</span>
              <input type="number" step="10" defaultValue={t.inactMinNet} onBlur={e => patch({ inactMinNet: Number(e.target.value) || 0 })} style={{ ...settingsInputStyle, width: 70, textAlign: 'center' }} />
              <span>$ net sur</span>
              <input type="number" min="1" defaultValue={t.inactWindow} onBlur={e => patch({ inactWindow: Number(e.target.value) || 1 })} style={{ ...settingsInputStyle, width: 56, textAlign: 'center' }} />
              <span>j glissants.</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <SettingsNumField label="Statut dormant après (j, vide = pas d'étape)" value={t.inactDormantDays} onCommit={v => patch({ inactDormantDays: v })} />
              <SettingsNumField label="Clôture définitive après (j)" value={t.inactCloseDays} onCommit={v => patch({ inactCloseDays: v || 30 })} />
            </div>
          </div>
        )}
      </div>
      ...
      {/* payout rule */}
      <div style={{ marginTop: 10, padding: 12, borderRadius: 4, background: 'var(--surface-2)' }}>
        <SettingsToggle label="Règle de payout" on={!!t.hasPayout} onClick={() => patch({ hasPayout: !t.hasPayout })} />
        {t.hasPayout && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 13, color: 'var(--ink-2)' }}>
              <span>Min.</span>
              <input type="number" min="1" defaultValue={t.payoutMinDays} onBlur={e => patch({ payoutMinDays: Number(e.target.value) || 0 })} style={{ ...settingsInputStyle, width: 52, textAlign: 'center' }} />
              <span>jours qualifiants à</span>
              <input type="number" step="10" defaultValue={t.payoutMinNet} onBlur={e => patch({ payoutMinNet: Number(e.target.value) || 0 })} style={{ ...settingsInputStyle, width: 70, textAlign: 'center' }} />
              <span>$ net (depuis le dernier payout).</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <SettingsNumField label="Jours de trading min. (total, optionnel)" value={t.payoutMinTradingDays} onCommit={v => patch({ payoutMinTradingDays: v })} />
              <SettingsNumField label="Consistance (%, 0 = désactivée)" value={t.consistencyPct} step={5} onCommit={v => patch({ consistencyPct: v || 0 })} />
              <SettingsNumField label="Nombre max de payouts" value={t.maxPayouts} onCommit={v => patch({ maxPayouts: v || 0 })} />
              <SettingsNumField label="Split (%)" value={t.payoutSplit} step={5} onCommit={v => patch({ payoutSplit: v || 100 })} />
            </div>

            {(t.payoutModel || 'scale') === 'scale' ? (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <SettingsNumField label="Filet de sécurité ($)" value={t.safetyNet} step={100} onCommit={v => patch({ safetyNet: v || 0 })} />
                  <SettingsNumField label="Filet applicable aux N premiers payouts (vide = toujours)" value={t.safetyNetMaxPayouts} onCommit={v => patch({ safetyNetMaxPayouts: v })} />
                  <SettingsNumField label="Solde min. pour demander ($)" value={t.payoutMinBalance} step={100} onCommit={v => patch({ payoutMinBalance: v || 0 })} />
                  <SettingsNumField label="Payout minimum ($)" value={t.payoutMin} step={50} onCommit={v => patch({ payoutMin: v || 0 })} />
                </div>
                <label style={{ display: 'block' }}>
                  ...
                  <input defaultValue={(t.payoutScale || []).join(', ')} placeholder="ex. 1500, 1500, 2000, 2500, 2500, 3000"
                    ...
                    style={{ ...settingsInputStyle, width: '100%' }} />
                </label>
                <SettingsToggle label="L'échelle expire (plus aucun plafond une fois épuisée, au lieu de répéter le dernier)" on={!!t.payoutCapExpires} onClick={() => patch({ payoutCapExpires: !t.payoutCapExpires })} />
                <label style={{ display: 'block' }}>
                  ...
                  <input defaultValue={t.payoutSplitNote || ''} placeholder="ex. 100 % sur les 25 000 $ premiers, puis 90 %…"
                    onBlur={e => patch({ payoutSplitNote: e.target.value })} style={{ ...settingsInputStyle, width: '100%' }} />
                </label>
              </>
            ) : (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <SettingsNumField label="% du profit versé" value={t.payoutPct} step={5} onCommit={v => patch({ payoutPct: v || 50 })} />
                  <SettingsNumField label="Plafond fixe ($)" value={t.payoutCap} step={100} onCommit={v => patch({ payoutCap: v || 0 })} />
                </div>
                <SettingsToggle label="Exige un profit net positif sur l'ensemble du cycle" on={!!t.requireOverallProfit} onClick={() => patch({ requireOverallProfit: !t.requireOverallProfit })} />
              </>
            )}
          </div>
        )}
      ...
      {/* eval-only fields */}
      <div style={{ marginTop: 10, padding: 12, borderRadius: 4, background: 'var(--surface-2)' }}>
        <SettingsToggle label="Compte d'évaluation (pas encore financé)" on={!!t.isEval} onClick={() => patch({ isEval: !t.isEval })} />
        {t.isEval && (
          <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <SettingsNumField label="Objectif de profit ($)" value={t.profitTarget} step={100} onCommit={v => patch({ profitTarget: v || 0 })} />
            <SettingsNumField label="Perte maximum ($)" value={t.maxLossLimit} step={100} onCommit={v => patch({ maxLossLimit: v || 0 })} />
            <SettingsNumField label="Consistance (%, 0 = désactivée)" value={t.evalConsistencyPct} step={5} onCommit={v => patch({ evalConsistencyPct: v || 0 })} />
            <div />
            <SettingsNumField label="Taille max (mini)" value={t.sizeCapMini} onCommit={v => patch({ sizeCapMini: v })} />
            <SettingsNumField label="Taille max (micros)" value={t.sizeCapMicro} onCommit={v => patch({ sizeCapMicro: v })} />
          </div>
        )}
      </div>

      <div style={{ marginTop: 10 }}>
        <SettingsNumField label="Frais d'activation, une fois (ex. PA Apex, optionnel)" value={t.activationFee} step={10} onCommit={v => patch({ activationFee: v })} width={180} />
      </div>
            ...
            <div key={fm.name} style={{ borderRadius: 4, background: 'var(--surface-2)', border: '1px solid var(--border)', overflow: 'hidden' }}>
              ...
              {open && (
                <div style={{ padding: '0 14px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {(fm.accountTypes || []).map(t => <TypeEditor key={t.id} firm={fm.name} t={t} onUpdate={updateType} onRemove={removeType} />)}
